...
import com.claro.data.RestServiceFACT;
...
public class CheckResource {
    ...
    public String validateUser(String cadena) {
        ...
        if ("1".equals(respuesta)) {
            ...
            String Token = dbManager.getTokenUserSession(dataUser.getUser(), "502");
            if (Token.equals("-2")){
    ...
    public String isUserLogged(@PathParam("userId") String user,@PathParam("token") String token) {
        try{
        DbModel dbManager = new DbModel();
        String isUserLogged = dbManager.isUserLogged(user, token, "502");
        return isUserLogged;
        } catch(Exception e){
            return "false";
        }
    ...
    public String CloseSessionUserLogged (@PathParam("userId") String user,@PathParam("token") String token) {
        try{
        DbModel dbManager = new DbModel();
        String CloseSessionUserLogged  = dbManager.CloseSessionUserLogged (user, token, "502");
    ...
    public String urlAPIMap() {
        DbModel dbManager = new DbModel();
        String url = DecryptCryptoJs.encrypt(dbManager.getParam("API_URL_GOOGLE_MAPS",502));
        return "\""+url+"\""; 
    }
    ...
    public String keyAPIMap() {
        DbModel dbManager = new DbModel();
        //System.out.println(dbManager.getParam("API_KEY_GOOGLE_MAPS",502));
        String key = DecryptCryptoJs.encrypt(dbManager.getParam("API_KEY_GOOGLE_MAPS",502));
        return "\""+key+"\"";
    }
    ...
    */
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("asociar-orden")
    public String asociarOrden(String cadena) throws JSONException {
        String asociarIFI = "{\"estado\":\"1\",\"msg\":\"NoInteraccion invalido debe ser tipo PISAOS o PISTATK.\"}";
        //try {
            //****************************************
            //Crear la orden en el facturador
            RestServiceFACT RestSRVFACT = new RestServiceFACT();
            Integer asociarRestFACT = RestSRVFACT.RestServiceFACT(cadena);
            //****************************************            
            if(asociarRestFACT==0)
            {
                DbModel dbManager = new DbModel();
    ...
    */
    ...
    //public String obtenerToken(String pais, String tecno) {
        DbModel dbManager = new DbModel();
        //DbModelFACT dbManager = new RestServiceFACT();
