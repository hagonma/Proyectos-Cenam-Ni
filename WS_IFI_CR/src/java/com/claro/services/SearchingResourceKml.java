...
public class SearchingResourceKml {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes("application/json")
    @Path("consultarFactibilidadAPITodas")
    public Response consultarFactibilidadAPITodas(String cadenaJson) throws UnsupportedEncodingException, IOException {
        //System.out.println(cadenaJson);
        String respuesta = this.controlador.consultarFactibilidadAPITodas(cadenaJson);
        return Response.ok(respuesta).header("Access-Control-Allow-Origin", "*")
                .header("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT, OPTIONS, HEAD")
                .header("Access-Control-Allow-Headers", "Content-Type, Accept, X-Requested-With").build();
