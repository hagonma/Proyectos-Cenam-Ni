...
public class CheckResource {
    ...
    public String validateUser(String cadena) {
        ...
        if ("1".equals(respuesta)) {
            ...
            String Token = dbManager.getTokenUserSession(dataUser.getUser(), "506");
            if (Token.equals("-2")){
    ...
    public String isUserLogged(@PathParam("userId") String user,@PathParam("token") String token) {
        try{
        DbModel dbManager = new DbModel();
        String isUserLogged = dbManager.isUserLogged(user, token, "506");
        return isUserLogged;
        } catch(Exception e){
            return "false";
        }
    ...
    public String CloseSessionUserLogged (@PathParam("userId") String user,@PathParam("token") String token) {
        try{
        DbModel dbManager = new DbModel();
        String CloseSessionUserLogged  = dbManager.CloseSessionUserLogged (user, token, "506");
    ...
    public String urlAPIMap() {
        DbModel dbManager = new DbModel();
        String url = DecryptCryptoJs.encrypt(dbManager.getParam("API_URL_GOOGLE_MAPS",506));
        return "\""+url+"\""; 
    }
    ...
    public String keyAPIMap() {
        DbModel dbManager = new DbModel();
        //System.out.println(dbManager.getParam("API_KEY_GOOGLE_MAPS",506));
        String key = DecryptCryptoJs.encrypt(dbManager.getParam("API_KEY_GOOGLE_MAPS",506));
        return "\""+key+"\"";
    }
