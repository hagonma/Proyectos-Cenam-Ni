...
public class DbModelCRM {
    ...
    private static final String DSSPNAME = "jdbc/crm-pa";
    ...
    public String getOrden(String msisdn) {
        ...
        try {
            String query = base.getParam("QUERY_CRM_GET_ORDEN_HZ",Integer.parseInt("507"));
    ...
    public String getStatusInteraction(String no_interaction) {
        ...
        try {
            String query = base.getParam("QUERY_OBTAIN_STATUS_ORDER",Integer.parseInt("507"));
            pstmt = conn.prepareStatement(query);
            pstmt.setString(1,no_interaction);
            rs = pstmt.executeQuery();
            if (rs.next()) {
                orden = rs.getString("STATUS_PORT_NEW");
                rs.close();
            }
        } catch (SQLException e) {
            Logger.getLogger(DbModel.class.getName()).log(Level.SEVERE, null,"Error al llamar la funcion PL/SQL." + e.getMessage());
