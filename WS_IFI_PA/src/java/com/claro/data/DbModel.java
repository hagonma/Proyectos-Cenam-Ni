...
public class DbModel {
    ...
    private static final String DSSPNAME = "jdbc/consulta-mapa";
    ...
    public String[] changeStatePortTrx(String P_NO_INTERACTION, String P_STATUS_PORT_OLD, String P_STATUS_PORT_NEW) {
         Connection conn;
        CallableStatement cs = null;
        String[] val= new String[2];
        val[0]="-999";
        val[1]="Error no se factible hacer el cambio del estado del puerto.";

        this.initialize();
        conn = getConnection();

        try {
            String query = "{call PR_IFI_CHANGE_STATE_PORT_TRX(?,?,?,?,?,?,?)}";
            cs = conn.prepareCall(query);
            cs.setString(1, "507");
            cs.setString(2, P_NO_INTERACTION);
            cs.setString(3, P_STATUS_PORT_OLD);
            cs.setString(4, P_STATUS_PORT_NEW);
            cs.setString(5, "ALTA_SERV");
            cs.registerOutParameter(6, java.sql.Types.VARCHAR);
            cs.registerOutParameter(7, java.sql.Types.VARCHAR);

            cs.execute();
            val[0]=cs.getString(6);
            val[1]=cs.getString(7);
