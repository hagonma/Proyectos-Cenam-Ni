...
public class CheckResource {
    ...
    public String validateUser(String cadena) {
        ...
        if ("1".equals(respuesta)) {
            ...
            String Token = dbManager.getTokenUserSession(dataUser.getUser(), "507");
            if (Token.equals("-2")){
    ...
    public String isUserLogged(@PathParam("userId") String user,@PathParam("token") String token) {
        try{
        DbModel dbManager = new DbModel();
        String isUserLogged = dbManager.isUserLogged(user, token, "507");
        return isUserLogged;
        } catch(Exception e){
            return "false";
        }
    ...
    public String CloseSessionUserLogged (@PathParam("userId") String user,@PathParam("token") String token) {
        try{
        DbModel dbManager = new DbModel();
        String CloseSessionUserLogged  = dbManager.CloseSessionUserLogged (user, token, "507");
    ...
    public String urlAPIMap() {
        DbModel dbManager = new DbModel();
        String url = DecryptCryptoJs.encrypt(dbManager.getParam("API_URL_GOOGLE_MAPS",507));
        return "\""+url+"\""; 
    }
    ...
    public String keyAPIMap() {
        DbModel dbManager = new DbModel();
        String key = DecryptCryptoJs.encrypt(dbManager.getParam("API_KEY_GOOGLE_MAPS",507));
        return "\""+key+"\"";
    }
